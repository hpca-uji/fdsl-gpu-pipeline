...
__global__ void compute_vertex(float *__restrict vx, float *__restrict vy,
                               ...
                               int *__restrict pos, const int nvertex,
...
int main()
{
    ...
    constexpr size_t vertexsize = sizeof(float) * totalvertex;
    constexpr size_t colorsize = sizeof(uint8_t) * norbits;
    constexpr size_t posize = sizeof(int) * 2;

    // Pointers to arrays on device
    uint8_t *d_imgarr, *d_colors;
    int *d_pos;
    float *d_vx, *d_vy, *d_nx, *d_ny;

    // Reserve space on device
    cudaMalloc(&d_imgarr, imgsize);
    cudaMemset(d_imgarr, 0, imgsize);
    cudaMalloc(&d_vx, vertexsize);
    cudaMalloc(&d_vy, vertexsize);
    cudaMalloc(&d_nx, vertexsize);
    cudaMalloc(&d_ny, vertexsize);
    ...
    for (int i = 0; i < WARMUP; i++)
    {
        ...
        // gen random
        rng<<<1, 1>>>(d_nx, d_ny, d_colors, d_pos, nvertex, norbits, res, seed);

        // Compute vertex
        int NUM_THREADS = nvertex;
        int NUM_BLOCKS = 1;
        compute_vertex<<<NUM_BLOCKS, NUM_THREADS>>>(d_vx, d_vy, d_nx, d_ny, d_pos, nvertex, norbits, ovalx, ovaly,
                                                    freq1, freq2, noisecoef, startrad, linewidth, res);

        // Draw vertex
        dim3 block(32, 8);
        dim3 grid((nvertex + 32 - 1) / 32, (norbits + 8 - 1) / 8);
        draw_vertex<<<grid, block>>>(d_imgarr, d_vx, d_vy, d_colors, nvertex, norbits, res);
    }
