...
__global__ void gen_color_pos(uint8_t *__restrict colors, int2 *__restrict pos, const int res, unsigned int norbits, unsigned int seed)
{
    ...
    {
        colors[i] = roundf(curand_uniform(&state) * 255.0f);
    }

    // Position
    float px = roundf(curand_uniform(&state) * res);
    float py = roundf(curand_uniform(&state) * res);
    pos[0] = make_int2(px, py);
}
...
__global__ void gen_noise(half2 *__restrict noise, const int nvertex, unsigned int seed)
{
    ...
    for (int i = 0; i < nvertex; i++)
    {
        ...
        noise[noiseidx + i] = __float22half2_rn(n);
    }
}
