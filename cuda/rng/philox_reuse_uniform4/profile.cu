...
__global__ void gen_color_pos(uint8_t *__restrict colors, int2 *__restrict pos, const int res, unsigned int norbits, unsigned int seed)
{
    ...
    for (int i = 0; i + 3 < norbits; i += 4) // Never have to check out of bounds in this loop
    {
...
int main()
{
    ...
    cudaMalloc(&d_noise, vertexsize_h);
    ...
    // Print profiling
    total_all = total_gen + total_compute + total_draw;
    printf("Average main_kernel time: %.4f ms\n", total_all / N);
    printf("\trng:    %.4f ms\n", total_gen / N);
    printf("\tcompute_vertex: %.4f ms\n", total_compute / N);
    printf("\tdraw_vertex:   %.4f ms\n", total_draw / N);
