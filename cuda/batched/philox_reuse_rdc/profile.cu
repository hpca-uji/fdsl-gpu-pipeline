...
__global__ void gen_color_pos(uint8_t *__restrict colors, int2 *__restrict pos, const int res, unsigned int norbits, unsigned int seed)
{

    // Init state
    curandStatePhilox4_32_10_t state;
    curand_init(seed, 0, 0ULL, &state);

    unsigned int i = 0;

    // Process 4 elements at a time
    for (i = 0; i + 3 < norbits; i += 4) // Never have to check out of bounds in this loop
    {
        float4 r = curand_uniform4(&state);
        colors[i + 0] = (uint8_t)roundf(r.x * 255.0f);
        colors[i + 1] = (uint8_t)roundf(r.y * 255.0f);
        colors[i + 2] = (uint8_t)roundf(r.z * 255.0f);
        colors[i + 3] = (uint8_t)roundf(r.w * 255.0f);
    }

    // Tail (0–3 remaining elements, at most one more float4)
    int remaining = norbits - i;
    if (remaining > 0)
    {
        float4 r = curand_uniform4(&state);
        ...
        for (unsigned int k = 0; k < remaining; ++k)
            colors[i + k] = (uint8_t)(vals[k] * 255.0f);
    }

    // Two random positions
    float4 p = curand_uniform4(&state);
    pos[0] = make_int2(roundf(p.x * res), roundf(p.y * res));
}

// <<<NUM_BLOCKS=norbits, NUM_THREADS=1>>>
__global__ void gen_noise(half2 *__restrict noise, const int nvertex, unsigned int seed)
{
    // Get orbitidx and idx in noise array
    int orbitidx = blockIdx.x * blockDim.x;
    int noiseidx = orbitidx * nvertex;

    // Init curand state
    curandStatePhilox4_32_10_t state;
    curand_init(seed, orbitidx, 0ULL, &state);

    // Loop over nvertex
    int i;
    float4 r;
    float x, y;
    for (i = 0; i < nvertex - 1; i += 2)
    {
        r = curand_uniform4(&state);
        x = noise1(r.x * 10000.0f, 1024, 0);
        y = noise1(r.y * 10000.0f, 1024, 0);
        noise[noiseidx + i] = __float22half2_rn(make_float2(x, y));
        x = noise1(r.w * 10000.0f, 1024, 0);
        y = noise1(r.z * 10000.0f, 1024, 0);
        noise[noiseidx + i + 1] = __float22half2_rn(make_float2(x, y));
    }

    if (i < nvertex)
    { // One vertex still missing
        float4 r = curand_uniform4(&state);
        float x = noise1(r.x * 10000.0f, 1024, 0);
        float y = noise1(r.y * 10000.0f, 1024, 0);
        noise[noiseidx + i] = __float22half2_rn(make_float2(x, y));
    }
}
                    ...
                    const int res, unsigned int seed)
{
    ...
    gen_noise<<<NUM_BLOCKS, NUM_THREADS, 0, cudaStreamFireAndForget>>>(noise, nvertex, seed);

    // Generate color pos
    NUM_THREADS = 1;
    NUM_BLOCKS = 1;
    gen_color_pos<<<1, 1, 0, cudaStreamFireAndForget>>>(colors, pos, res, norbits, seed);
}
...
__global__ void genvatom_batch(uint8_t *d_imgarr_base, float2 *d_vertex_base,
                               ...
                               int2 *d_pos_base, int nvertex, int norbits, float ovalx, float ovaly,
                               int freq1, int freq2, float noisecoef, int startrad, float linewidth, int res,
                               int base_seed)
{
    ...
    // Sizes for one image
    const int totalvertex = nvertex * norbits;
    const size_t img_offset = (size_t)img_idx * res * res;
    const size_t vertex_offset = (size_t)img_idx * totalvertex;
    const size_t colors_offset = (size_t)img_idx * norbits;
    ...
    unsigned int current_seed = base_seed + img_idx;

    genvatom_image<<<1, 1, 0, cudaStreamFireAndForget>>>(imgarr_ptr, vertex_ptr, noise_ptr, colors_ptr,
                                                         pos_ptr, nvertex, norbits, ovalx, ovaly, freq1, freq2,
                                                         noisecoef, startrad, linewidth, res, current_seed);
}
...
int main()
{
    ...
    constexpr float linewidth = 0.002f;
    ...
    constexpr size_t vertexsize_h = sizeof(half2) * totalvertex;
    ...
    for (int i = 0; i < WARMUP; i++)
    {
        ...
        genvatom_batch<<<NUM_BLOCKS, NUM_THREADS>>>(
            ...
            nvertex, norbits, ovalx, ovaly, freq1, freq2, noisecoef,
            startrad, linewidth, res, seed);
        cudaDeviceSynchronize();
    }

    // Init vars for timers
    float total_all = 0.0f;
    cudaEvent_t s, e;

    // Call cuda kernel in for loop
    for (int i = 0; i < N; i++)
    {
        // Gen random
        cudaEventCreate(&s);
    ...
    printf("Average main_kernel time: %.4f ms\n", total_all / (N * batch_size));
